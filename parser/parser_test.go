...
func Test_HashString(t *testing.T) {
	input := []l.Token{
		{Type: l.SYMBOL, Content: "getdvar"},
		{Type: l.OPEN_PAREN, Content: "("},
		{Type: l.HASH_STRING, Content: "mapname"},
		{Type: l.CLOSE_PAREN, Content: ")"},
		{Type: l.TERMINATOR, Content: ";"},
	}

	target := []testNode{
		{"function_call", p.NodeData{FunctionName: "getdvar"}, []testNode{
...
func Test_HashStringInSwitch(t *testing.T) {
	input := []l.Token{
		{Type: l.SYMBOL, Content: "switch"},
		{Type: l.OPEN_PAREN, Content: "("},
		{Type: l.SYMBOL, Content: "getdvar"},
	...
	result, diags := parseTokens(t, input)
	assert.Equal(t, 0, len(diags))
	assert.Equal(t, 1, len(result))
	assert.Equal(t, "switch_statement", result[0].Type)
}
